...
└────────┬────────┘
         │
┌────────┴────────┐
│  Leaf (实验电脑) │ ← 自动上传实验数据 (DATA 区)
└─────────────────┘
         │
┌────────┴────────┐
│  PC (个人设备)   │ ← 只读数据区 + 上传/下载协作区
└─────────────────┘
```
...
|--------|---------------|------------------------|
| **Leaf Agent** | ✅ **只能由它上传** | ❌ 不上传 |
| **PC Client** | ❌ 只读（查看/下载） | ✅ 可上传/下载 |

> ⚠️ 实验数据（DATA 区）只能由实验室电脑上的 Leaf Agent 上传，确保数据来源的可信度。PC 客户端**无法**上传到数据区。
