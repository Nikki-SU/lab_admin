...
| 场景 | 风险等级 | 影响 |
|------|----------|------|
| **多 Leaf Agent 同时上传同一路径文件** | 🔴 高 | 后上传的会覆盖先上传的，且标记为编辑 |
| **同一文件在短时间内多次修改** | 🟡 中等 | 产生大量版本，占用存储空间 |
| **网络中断导致部分上传** | 🟡 中等 | 产生不完整的文件记录 |
...
**短期方案（运维措施）：**
1. 为不同 Leaf Agent 分配不同的上传路径前缀
2. 配置合理的文件监控延迟（当前 `time.sleep(1)`）
3. 定期审查 `edited=True` 的文件列表

**配置建议（Leaf Agent）：**
```env
# leaf-agent/.env
DEVICE_NAME=lab-device-001  # 确保每个设备名称唯一
WATCH_DIR=./watch
```
