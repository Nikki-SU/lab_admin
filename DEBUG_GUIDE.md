...
| 客户端 | 数据区 (DATA) | 协作区 (COLLABORATION) |
|--------|---------------|------------------------|
| **Hub Server** | ❌ 不直接上传 | ❌ 不直接上传 |
| **Leaf Agent** (实验室电脑) | ✅ **只能由它上传** | ❌ 不上传 |
| **PC Client** (个人设备) | ❌ 只读（查看/下载） | ✅ 可上传/下载 |
...
测试脚本会模拟 **Leaf Agent** 上传数据到 DATA 区。
...
2. 登录默认账号：
   ...
   - 密码：`admin123`
3. **协作区**：可以正常上传/下载文件
4. **数据区**：只读模式，只能查看和下载 Leaf Agent 上传的文件
5. 在 `leaf-agent/watch` 文件夹中创建/修改文件来触发 Leaf Agent 上传

**方法 C - 验证 PC 客户端被禁止上传到数据区：**
如果尝试通过 PC 客户端上传到数据区，会收到 403 错误：
```
数据区(实验数据)只允许从 Leaf Agent (实验室电脑) 上传，PC 客户端无法上传到该区域
```
